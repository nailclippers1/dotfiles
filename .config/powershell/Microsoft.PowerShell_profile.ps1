# to overwrite the existing profile, create new profile with "ni $profile" and replace its content.
...
#git add -A && git commit -m "$(date)" && git push origin $(git branch | sed -n -e "s/^\* \(.*\)/\2/p")
function gitup(){
